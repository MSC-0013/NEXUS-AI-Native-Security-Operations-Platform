...
CREATE INDEX IF NOT EXISTS idx_security_events_org_type_time
...
-- endpoints
CREATE INDEX IF NOT EXISTS idx_endpoints_org_status
  ON endpoints (organization_id, status);

CREATE INDEX IF NOT EXISTS idx_endpoints_org_isolated
  ON endpoints (organization_id, is_isolated)
  WHERE is_isolated = true;

-- network_flows
CREATE INDEX IF NOT EXISTS idx_network_flows_org_malicious
  ON network_flows (organization_id, is_malicious, flow_start DESC)
  WHERE is_malicious = true;

CREATE INDEX IF NOT EXISTS idx_network_flows_org_start
  ON network_flows (organization_id, flow_start DESC);

-- dns_queries
CREATE INDEX IF NOT EXISTS idx_dns_queries_org_dga
  ON dns_queries (organization_id, is_dga)
  WHERE is_dga = true;

-- asset_vulnerabilities
CREATE INDEX IF NOT EXISTS idx_asset_vulns_org_status
